...
;(function($){
	...
	var animate = function($el){
		var _opts = $el.data(ns.data),
			direction = $el.hasClass(_opts.current) ? 'Up' : 'Down';
			
		$el.find(_opts.content)['slide' + direction](_opts.duration, function(){
			toggleCurrent($el);
		});
	};
	
	var toggleCurrent = function($el){
		var _opts = $el.data(ns.data),
			func  = {'Open' : 'add', 'Close': 'remove'},
			which = $el.hasClass(_opts.current) ? 'Close' : 'Open';
			
		$el[func[which] + 'Class'](_opts.current + ' ' + ns.opened);
		callback(_opts['on' + which]);
	};
